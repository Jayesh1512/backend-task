document.addEventListener('DOMContentLoaded', () => {
  ...
  pnlForm.addEventListener('submit', async (e) => {
    ...
    const submitBtnInner = '<span>Send Request</span><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3L13 8L8 13M13 8H3" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
    const form = new FormData(pnlForm);
    const wallet = form.get('wallet');

    // Date-only mode: use the date inputs (YYYY-MM-DD)
    const start = form.get('start');
    const end = form.get('end');

    try {
      if (!wallet || !start || !end) {
        showResponse({ error: 'Wallet, start and end are required and must be valid.' }, 400);
        return;
      }
  ...
  // Date-only mode enforced — no UI toggles required
